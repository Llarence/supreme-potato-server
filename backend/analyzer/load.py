...
def match_to_tensors(match):
    ...
    blue_output = tf.convert_to_tensor(blue_output, dtype=tf.float32)
...
# Remove the scores required for this function
def hypo_match_to_tensors(match):
    if len(match[0]) > 0:
        blue_robots = sum([one_hot_cache[teams_to_indices[team]] for team in match[0]])
    else:
        blue_robots = tf.zeros(len(team_indices))

    if len(match[1]) > 0:
        red_robots = sum([one_hot_cache[teams_to_indices[team]] for team in match[1]])
    else:
        red_robots = tf.zeros(len(team_indices))
    ...
    return (tf.cast(tf.stack((blue_robots, red_robots, match_meta)), dtype=tf.float32), tf.cast(tf.stack((red_robots, blue_robots, match_meta)), dtype=tf.float32))
