...
def get_hypo_match(response: fastapi.Response, elim: bool, week: int, detail: int = 0, blues: list[int] = fastapi.Query(None), reds: list[int] = fastapi.Query(None)):
    ...
    #try:
    if not blues is None:
        blueTeams = ['frc' + str(blue) for blue in blues]
    else:
        blueTeams = []

    if not reds is None:
        redTeams = ['frc' + str(red) for red in reds]
    else:
        redTeams = []

    match = [blueTeams, redTeams, [elim, week]]
    match_tensors = load.hypo_match_to_tensors(match)
    tensor = tf.stack((match_tensors[0], match_tensors[1]))
